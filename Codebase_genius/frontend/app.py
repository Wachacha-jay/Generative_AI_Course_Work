...
from agents.code_genius import CodeGenius
...
def main():
    ...
    if 'code_genius' not in st.session_state:
        st.session_state.code_genius = CodeGenius()
    ...
    if analyze_button and repo_url:
        with st.spinner("Analyzing repository..."):
            try:
                # Perform analysis
                result = st.session_state.code_genius.analyze_repository(
                    repo_url=repo_url,
...
def display_analysis_results(result):
    ...
    if st.button("📥 Download Markdown Documentation"):
        # Save documentation
        output_path = st.session_state.code_genius.doc_genie.save_documentation(
            doc, 
            st.session_state.code_genius._extract_repo_name(result.repo_map.repo_info.get('url', ''))
        )
...
def display_language_detection_info():
    ...
    languages = [
        'python', 'jac', 'javascript', 'java', 'cpp', 'rust', 'go'
    ]
    
    for lang in languages:
        info = detector.get_language_info(lang)
        
        with st.expander(f"🔤 {info['name']}"):
            st.markdown(f"**Description:** {info['description']}")
            st.markdown(f"**Parser:** {info['parser']}")
